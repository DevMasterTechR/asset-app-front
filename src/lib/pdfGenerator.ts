...
export const generateSecurityReportPDF = async (
  ...
  const colWidths = [22, 32, 25, 12, 35, 22, 28, 30, 44];
  ...
  const processDevice = async (device: SecurityReport, idx: number) => {
    ...
    doc.text(brandModel.substring(0, 20), x, y + 5.5);
    x += colWidths[1];
    
    // Categoría
    doc.text((device.category || '-').substring(0, 15), x, y + 5.5);
    x += colWidths[2];
    ...
    // Ubicación
    doc.text((device.location || '-').substring(0, 20), x, y + 5.5);
    x += colWidths[4];
    ...
    // Sucursal
    doc.text((device.branchName || '-').substring(0, 15), x, y + 5.5);
    x += colWidths[6];
    
    // Observación
    doc.text((device.notes || '-').substring(0, 15), x, y + 5.5);
    x += colWidths[7];
